...
def svm_loss(x, y):
    ...
    scores = x # (N, C); x is already scores, not a raw input
    correct_class_scores = scores[np.arange(len(x)), y] # (N, )
    margin = scores - correct_class_scores.reshape(-1, 1) + 1 # matrix of margins, (N, C)
    loss = ((margin * (margin > 0)).sum() - len(x)) / len(x)

    dx = (margin > 0).astype(np.float64)
    dx[np.arange(len(x)), y] -= (margin > 0).sum(axis=1) # for correct classes
    dx /= len(x)
